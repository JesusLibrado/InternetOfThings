...
class Fitocsc(Frame):
    ...
    def setMai(self):
        os.system("python FitoMaiGui.py")

    def setMav(self):
        os.system("python FitoMavGui.py")

    def setAct(self):
        os.system("python FitoAct.py")
    ...
    def getDeviceData(self):
        ...
        self.date = data["date"]
        #data['calle'] + ", " + data['colonia'] + " " + data['ciudad'] +  " " + data['estado'] + " " + "mexico"
        #self.address =   data['estado'] + "+" + "mexico"
        #calle = 'Via Atlixcayotl 2301 Reserva territorial'
        #data['calle'] + ", " + data['colonia'] 
        #ciudad = 'Puebla Mexico'
        #data['ciudad'] +  " " + data['estado'] + ", " + "mexico"

        lblDeviceName = Label(self, text="Id: " + self.deviceid)
        lblDeviceName.pack()
        lblDeviceName = Label(self, text="Name: " + self.name)
        lblDeviceName.pack()
        lblDeviceName = Label(self, text="Code: " + self.code)
        lblDeviceName.pack()
        lblDeviceName = Label(self, text= self.description)
        lblDeviceName.pack()
        lblDeviceName = Label(self, text= self.date)
        lblDeviceName.pack()
        lblDeviceName = Label(self, text= self.location)
        lblDeviceName.pack()


        myGeolocation = FitoGeolocation();
        
        lat, lng = myGeolocation.getGeolocation(self.location);
        self.lat = lat
        self.lng = lng
        #tkMessageBox.showinfo("FitoSmart - Geolocation", myMsg)


        lblAddress = Label(self, text= "lat: " + str(self.lat) + " lng: " + str(self.lng))
        lblAddress.pack()
